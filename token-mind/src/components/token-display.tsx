...
import { TokenPriceHistory } from "./token-graph"
import { mapTokenDataToDetails } from "@/lib/data-formatter"
...
export default function TokenDisplay({ token }: { token: TokenResponse['data'] }) {
  const [copied, setCopied] = useState(false)
  const data = mapTokenDataToDetails(token).body;
  ...
  return (
    <main className="w-full  px-2 py-2 text-[#f5f7f8] sm:px-3 sm:py-3">
      <div className="space-y-3">
        <Card className="overflow-hidden  shadow-xl shadow-black/20">
          <CardHeader className="border-b  p-5 sm:p-7">
            <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
              <div className="flex min-w-0 items-center gap-4">
                <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-[#2b3439]  text-2xl font-semibold text-[#70e2b2]">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {data.isVerifiedContract && <span className="inline-flex items-center gap-1 rounded-full  px-2 py-1 text-xs font-medium text-[#7cf0b8]"><ShieldCheck className="h-3.5 w-3.5" /> Verified</span>}
                  </div>
                  <p className="mt-1 text-sm text-[#8f9aa1]">{`${data.symbol}`} <span className="px-1 text-[#465057]">·</span> {data.standard ?? "Token"}</p>
                </div>
              </div>
              <div className="text-left md:text-right"><p className="text-xs uppercase tracking-[0.16em] text-[#77838a]">Current price</p><p className="mt-1 text-3xl font-semibold tracking-tight text-[#7cf0b8]">{formatCurrency(currentPrice)}</p></div>
            </div>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="min-w-0 flex-1 rounded-xl border px-3 py-2.5"><p className="mb-1 text-[10px] uppercase tracking-[0.16em] text-[#6f7a80]">Mint address</p><code className="block truncate text-xs text-[#c4cdd1]" title={address}>{shortAddress(address)}</code></div>
          ...
          <CardContent className="space-y-7 ">
            <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border sm:grid-cols-4">{stats.map(([label, value]) => <div key={label} className="p-4"><p className="text-xs text-[#78848b]">{label}</p><p className="mt-2 truncate text-sm font-medium text-[#e8edef]">{value}</p></div>)}</div>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
              <section><h2 className="mb-3 text-sm font-semibold text-[#e9eff0]">Explore links</h2>{links.length ? <div className="flex flex-wrap gap-2">{links.map(([label, href, Icon]) => <a key={label} href={href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs text-[#b9c5c9] transition hover:border-[#4b8f70] hover:text-[#7cf0b8]"><Icon className="h-3.5 w-3.5" />{label}</a>)}</div> : <p className="text-sm text-[#78848b]">No external links available.</p>}</section>
            </div>
          </CardContent>
        </Card>
        
        <TokenPriceHistory token={token} symbol={data.symbol}  />
      </div>
    </main>
  )
}
